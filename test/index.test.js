...
describe('store test', () => {
  ...
  it('should create a file', async () => {
    const res = await broker.call(`storage.create`, {
      ...DEFAULT_DRIVE,
      path: 'hi.txt',
    });
    expect(res).to.has.property('name', 'hi.txt');

    const res2 = await broker.call(`storage.create`, {
      ...DEFAULT_DRIVE,
      path: 'ho.txt',
      data: FileCursor(resolve('./package.json')),
    });
    expect(await res2.data.toText().indexOf('version')).to.be.not.eq(-1);
  });

  it('should list entries', async () => {
    const res = await broker.call(`storage.list`, {
      ...DEFAULT_DRIVE,
      path: '.',
    });
  ...
  it('should compress and extract a directory', async () => {
    const res = await broker.call(`storage.compress`, {
      ...DEFAULT_DRIVE,
      from: 'ho.txt',
    });
    expect(res).to.has.property('path', '/ho.zip');

    const res2 = await broker.call(`storage.extract`, {
      ...DEFAULT_DRIVE,
      from: 'ho.zip',
    });
    expect(res2).to.has.property('path', '/ho');

    const res3 = await broker.call(`storage.compress`, {
      ...DEFAULT_DRIVE,
      from: 'ho',
      to: 'bar/bar.zip',
    });
    ...
    const res4 = await broker.call(`storage.extract`, {
      ...DEFAULT_DRIVE,
      from: 'bar/bar.zip',
      to: 'ho2',
    });
  ...
  it('should move entries', async () => {
    const res = await broker.call(`storage.move`, {
      ...DEFAULT_DRIVE,
      from: 'bar/bar.zip',
      to: 'gao.zip',
    });
    ...
    const res2 = await broker.call(`storage.move`, {
      ...DEFAULT_DRIVE,
      from: 'ho2/ho',
      to: 'bar/hoho',
    });
    ...
    const res3 = await broker.call(`storage.move`, {
      ...DEFAULT_DRIVE,
      from: 'ho.zip',
      to: 'bar/hoho.zip',
    });
    ...
    try {
      await broker.call(`storage.move`, {
        ...DEFAULT_DRIVE,
        from: 'bar',
        to: 'bar/hoho/boo',
      });
  ...
  it('should delete an entry', async () => {
    const res = await broker.call(`storage.remove`, {
      ...DEFAULT_DRIVE,
      path: 'hi.txt',
    });
    expect(res).to.has.property('name', 'hi.txt');

    const res2 = await broker.call(`storage.list`, {
      ...DEFAULT_DRIVE,
      path: '.',
    });
  ...
  it('should deploy', async () => {
    const newPath = join(root, 'dst.zip');

    await FileCursor(
      join(root, DEFAULT_DRIVE.spaceId, DEFAULT_DRIVE.driveName, 'gao.zip')
    ).copyTo(newPath);

    const res = await broker.call(`storage.deploy`, {
      ...DEFAULT_DRIVE,
      data: FileCursor(newPath),
    });
